...
function sortRadix(shuffledList, rainbow) {
  ...
  const temp = Array.from(Array(10), () => Array());
  const maxlength = 3;
  ...
  for (let digit = 0; digit < maxlength; digit++) {
    ...
    shuffledList.map((element, curidx, self) => {
      ...
      const e1 = document.getElementById(`element${curidx}`);
      temp[number].push(e1);
    });
    ...
    queue.map((array, arrayidx, selfarr) => {
      while (array.length) {
        ...
        const changedidx = temp[arrayidx].shift();
        swpradix.push({e1 : idx - 1,  e2: shuffledList[idx - 1], color : rainbow[shuffledList[idx - 1]]});
      }
    });
  }
  return swpradix;
...
function runSwapRadix(swpradix) {
  ...
  swpradix.map((element, curidx, self) => {
    setTimeout(() => {
      ...
      const e1 = document.getElementById(`element${element.e1}`);      
      const e2 = element.e2;
      const color = element.color;

      let e2height = e2;
      e1.style.height = `${e2height}px`;
