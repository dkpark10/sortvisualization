...
function sortRadix(shuffledList) {
  ...
  const swparr = Array();
  const idqueue = Array.from(Array(10), () => Array());
  ...
  for (let digit = 0; digit < maxlength; digit++) {
    ...
    shuffledList.map((element, curidx, self) => {
      ...
      idqueue[number].push(`element${curidx}`);
    });

    let idx = 0;
    queue.map((array, arrayidx, selfarr) => {
      while (array.length) {
        const e2id = idqueue[arrayidx].shift();
        if(`element${idx}` !== e2id){
          swparr.push({ e1: `element${idx}`, e2: e2id });
        }
...
window.onload = function () {
  ...
  const swparr = sortHeap(shuffledList);
  console.log(swparr);
